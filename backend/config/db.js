 ...
 * Falls back to MongoMemoryServer if local MongoDB daemon is not running.
 */
const connectDB = async () => {
  try {
    const conn = await mongoose.connect(env.MONGO_URI, {
      autoIndex: true,
      serverSelectionTimeoutMS: 3000, // Quick timeout for fallback check
    });

    console.log(`[Database] MongoDB Connected: ${conn.connection.host}`);
    return conn;
  } catch (error) {
    console.warn(`[Database Warning] Primary connection failed (${error.message}). Booting MongoMemoryServer...`);
    ...
    try {
      ...
      // Auto-seed initial data in memory server
      await seedInMemoryDatabase();
      return conn;
    } catch (fallbackError) {
      console.error(`[Database Critical Error] Failed to boot fallback DB: ${fallbackError.message}`);
      process.exit(1);
    }
  }
};
...
/**
 * Helper to seed sample data into memory database on startup
 */
const seedInMemoryDatabase = async () => {
  try {
    ...
    // Create Sample Indian Market Inventory in INR
    ...
    console.log('[Database] Auto-seeded demo accounts & Indian market vehicle inventory successfully.');
  } catch (err) {
